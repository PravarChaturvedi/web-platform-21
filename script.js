...
//3:8
const num = [2, 4, 6, 8];
num.forEach((num, index) => console.log(`${index}:${num * 2}`));
...
cars.forEach((car, index) => {
  const color = car.color.join(' ');
  console.log(`${index + 1}. "${car.name} ${color} ${car.capacity}"`);
});
...
console.log('Bike property exists:', hasBike);
console.log('House property exists:', hasHouse);

// 3. Using dot and bracket notation get the output as "Mr Ajay Kumar travels by plane called Air123".
const output = `${person.name} travels by ${person.vehicles['plane']} called ${person.vehicles.plane}`;
console.log(output);
